...
$(function(){
	...
	$("#floor1 .flList li:first").addClass("flListStyle");
	$("#floor2 .flList li:first").addClass("flListStyle");
	$("#floor3 .flList li:first").addClass("flListStyle");
		...
		if(typeId != null){
			...
			var adata = {
				...
				$.ajax({
		            ...
		            data: data,
		            url: "/fruit/findByType?typeId=" + typeId,
		            success: function (res) {
		            	var content = "";
		                if (res != null) {
		                	
		                	for(var i=0;i<res.length;i++){
		                		console.log(res[i].fruitName);
		                		content+='<dl>';
		                		content+='<dt><a href="${pageContext.request.contextPath}/toProInfo?fruitId=${res[i].fruitId }"><img src="'+res[i].fruitImage+'" width="132" height="129" /></a></dt>';
		                		content+='<dd>'+res[i].fruitName+'</dd>';
		                		content+='<dd class="cheng">￥'+res[i].fruitPrice+' / '+res[i].fruitUnit+'</dd>';
		                		content+='</dl>';
		                		if(i==7){
		                			break;
		                		}
		                	}
		                	content+='<div class="clears"></div>';
		                	
		                	
		                	$("#floor1 .frProList").html(content);
		                } else {
		                	alert("什么都没有！");
		                }
		            },
	...
	$("#floor2 .flList li").click(function(){
		...
		var providerId = $(this).attr("index");
		...
		if(providerId!=null){
			...
			$.ajax({
	            ...
	            contentType: "application/json",
	            data: data,
	            url: "/fruit/findByProvider?providerId=" + providerId,
	            success: function (res) {
	            	var content = "";
	                if (res != null) {
		...
		$(this).addClass("flListStyle").siblings("li").removeClass("flListStyle");
		var index2=$(this).index();
		$("#floor2 .frProList li").eq(index2).show().siblings(".frProList").hide();
	})	
